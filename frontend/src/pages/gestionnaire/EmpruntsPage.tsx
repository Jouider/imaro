    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div className="flex items-start gap-3">
        <div className="flex size-10 items-center justify-center rounded-xl bg-[var(--color-imaro-primary)]/10">
          <Banknote className="size-5 text-[var(--color-imaro-primary)]" />
        </div>
                ...
                <TableRow key={e.id}>
                  ...
                  <TableCell className="text-xs text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      {dt.format(new Date(e.date_debut))} →{' '}
                      {dt.format(new Date(e.date_fin))}
                    </div>
                    <span className="text-[10px]">
                      {e.duree_mois} mois · {e.taux_interet}%
                    </span>
                  </TableCell>
                  <TableCell className="text-right tabular-nums text-sm">
                    {fmt.format(e.montant_initial)} DH
                  </TableCell>
                  <TableCell className="text-right tabular-nums text-sm">
                    {fmt.format(e.mensualite)} DH
                  </TableCell>
                  <TableCell className="text-right tabular-nums text-sm font-medium text-orange-600">
                    {fmt.format(e.reste)} DH
                  </TableCell>
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={cn('text-[10px]', STATUT_CLS[e.statut])}
                    >
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      <Button
                        ...
                        className="size-7"
                        onClick={() => {
                          if (confirm(`Supprimer l'emprunt « ${e.libelle} » ?`))
          ...
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, taux_interet: Number(e.target.value) })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  min="1"
                  value={draft.duree_mois || ''}
                  onChange={(e) =>
                    setDraft({ ...draft, duree_mois: Number(e.target.value) })
                  }
                />
              </div>
            </div>
            <div>
              ...
              <Input
                ...
                min="0"
                step="0.01"
                value={draft.mensualite || ''}
                onChange={(e) =>
                  setDraft({ ...draft, mensualite: Number(e.target.value) })
                }
              />
            </div>
          </div>
...
function Kpi({
  ...
  const tones = {
    primary: 'text-[var(--color-imaro-primary)]',
