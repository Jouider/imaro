...
export function EquipementsPage() {
  ...
  const amortissementPct =
    totals.valeurAcquisition > 0
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border bg-card p-4">
          ...
          <p className="text-2xl font-bold tracking-tight">
            {fmt.format(totals.valeurAcquisition)} DH
          </p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <div className="mb-2 flex items-center gap-2">
          ...
          <p className="text-2xl font-bold tracking-tight text-green-600">
            {fmt.format(totals.valeurNette)} DH
          </p>
        </div>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {equipementsQ.isLoading ? (
              ...
              equipements.map((e) => {
                const cat =
                  EQUIPEMENT_CATEGORIES.find((c) => c.value === e.categorie)
                    ?.label ?? e.categorie
                return (
                  <TableRow key={e.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground">
                      {dt.format(new Date(e.date_acquisition))}
                    </TableCell>
                    <TableCell className="text-right tabular-nums text-sm">
                      {fmt.format(e.valeur_acquisition)} DH
                    </TableCell>
                    <TableCell className="text-right tabular-nums text-sm font-medium text-green-600">
                      {fmt.format(e.valeur_nette)} DH
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="outline"
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setDraft({ ...draft, categorie: v as EquipementCategorie })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                ...
                <Input
                  id="date"
                  type="date"
                  value={draft.date_acquisition}
                  onChange={(e) =>
                    setDraft({ ...draft, date_acquisition: e.target.value })
                  }
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  min="0"
                  step="0.01"
                  value={draft.valeur_acquisition || ''}
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      valeur_acquisition: Number(e.target.value),
                    })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  type="number"
                  min="1"
                  value={draft.duree_amortissement_mois || ''}
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      duree_amortissement_mois: Number(e.target.value),
                    })
                  }
                />
              </div>
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              ...
              <Switch
                checked={draft.actif}
                onCheckedChange={(v) => setDraft({ ...draft, actif: v })}
              />
            </div>
            <div>
              <Label htmlFor="notes">
