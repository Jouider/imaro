...
const MOCK_EQUIPEMENTS: Equipement[] = [
  {
    id: 1,
    residence_id: 1,
    designation: 'Ascenseur principal Otis',
    categorie: 'ascenseur',
  ...
  },
  {
    id: 2,
    residence_id: 1,
    designation: 'Système vidéosurveillance Hikvision',
    categorie: 'videosurveillance',
    date_acquisition: '2022-09-01',
    valeur_acquisition: 35000,
  ...
  },
  {
    id: 3,
    residence_id: 1,
    designation: 'Pompe de surpression',
    categorie: 'plomberie',
    date_acquisition: '2020-06-10',
    valeur_acquisition: 12000,
    duree_amortissement_mois: 120,
    valeur_nette: 7500,
    actif: true,
  },
  {
    id: 4,
    residence_id: 1,
    designation: 'Portail électrique principal',
    categorie: 'securite',
    date_acquisition: '2017-01-20',
    valeur_acquisition: 28000,
    duree_amortissement_mois: 180,
    valeur_nette: 12000,
    actif: true,
  },
]
...
export async function deleteEquipement(id: number): Promise<void> {
  return withMock(async () => {
    await api.delete(`/gestionnaire/equipements/${id}`)
  }, undefined)
}
...
export type CreateEmpruntInput = Omit<
  Emprunt,
  'id' | 'paye_cumule' | 'paye_exercice' | 'reste' | 'statut'
>
...
const MOCK_EMPRUNTS: Emprunt[] = [
  {
    id: 1,
    residence_id: 1,
    libelle: 'Réfection toiture 2023',
...
export async function deleteEmprunt(id: number): Promise<void> {
  return withMock(async () => {
    await api.delete(`/gestionnaire/emprunts/${id}`)
  }, undefined)
}
...
const MOCK_TRAVAUX: TravauxExceptionnel[] = [
  {
    ...
    montant_vote: 95000,
    montant_engage: 92000,
    montant_regle: 60000,
    date_debut: '2025-09-01',
    date_fin_prevue: '2025-12-15',
    statut: 'en_cours',
  },
  {
    id: 2,
    ...
    montant_vote: 140000,
    montant_engage: 138000,
    montant_regle: 138000,
    date_debut: '2025-01-10',
    date_fin_prevue: '2025-02-15',
    date_fin_reelle: '2025-02-20',
    statut: 'termine',
  },
]

export async function getTravauxExceptionnels(
  residenceId: number,
): Promise<TravauxExceptionnel[]> {
  return withMock(
    async () => {
      const res = await api.get<
        ApiEnvelope<{ travaux: TravauxExceptionnel[] }>
...
export async function createTravaux(
  residenceId: number,
  input: CreateTravauxInput,
): Promise<TravauxExceptionnel> {
  return withMock(
    async () => {
  ...
  return withMock(
    async () => {
      const res = await api.put<ApiEnvelope<{ travaux: TravauxExceptionnel }>>(
        `/gestionnaire/travaux-exceptionnels/${id}`,
        patch,
      )
      return res.data.data.travaux
    },
...
export async function deleteTravaux(id: number): Promise<void> {
  return withMock(async () => {
    await api.delete(`/gestionnaire/travaux-exceptionnels/${id}`)
  }, undefined)
}
...
const MOCK_RECETTES: AutreRecette[] = [
  {
    ...
    exercice: 2026,
    date: '2026-01-05',
    libelle: 'Loyer parking C-12 janvier',
  ...
  },
  {
    ...
    exercice: 2026,
    date: '2026-01-15',
    libelle: 'Loyer antenne Orange Q1',
    categorie: 'location_antenne',
    montant: 1200,
    payeur: 'Orange Maroc',
  },
  {
    ...
    exercice: 2026,
    date: '2026-02-08',
    libelle: 'Indemnité dégât des eaux',
    categorie: 'indemnite_assurance',
    montant: 3500,
    payeur: 'AXA Assurances',
  },
]

export async function getAutresRecettes(
  residenceId: number,
  exercice?: number,
): Promise<AutreRecette[]> {
  return withMock(
    async () => {
    ...
    MOCK_RECETTES.filter(
      (r) =>
        r.residence_id === residenceId &&
        (exercice === undefined || r.exercice === exercice),
    ),
  )
}

export async function createAutreRecette(
  residenceId: number,
...
export async function updateAutreRecette(
  id: number,
  ...
  return withMock(
    async () => {
      const res = await api.put<ApiEnvelope<{ recette: AutreRecette }>>(
        `/gestionnaire/autres-recettes/${id}`,
        patch,
      )
      return res.data.data.recette
    },
...
export async function deleteAutreRecette(id: number): Promise<void> {
  return withMock(async () => {
    await api.delete(`/gestionnaire/autres-recettes/${id}`)
  }, undefined)
}
...
export type RemboursementMotif =
  | 'trop_percu'
  | 'erreur_appel'
  | 'indemnite'
  | 'autre'
...
const MOCK_REMBOURSEMENTS: Remboursement[] = [
  {
    ...
    lot_numero: 'A-02',
    motif: 'trop_percu',
    description: 'Double versement appel Q1 2026',
    montant: 850,
  ...
  },
  {
    ...
    lot_numero: 'A-01',
    motif: 'indemnite',
    description: 'Dégât eau facturé à tort',
    montant: 1200,
...
export async function getRemboursements(
  residenceId: number,
): Promise<Remboursement[]> {
  return withMock(
    async () => {
      const res = await api.get<
        ApiEnvelope<{ remboursements: Remboursement[] }>
  ...
  return withMock(
    async () => {
      const res = await api.put<ApiEnvelope<{ remboursement: Remboursement }>>(
        `/gestionnaire/remboursements/${id}`,
        patch,
      )
      return res.data.data.remboursement
    },
...
export async function deleteRemboursement(id: number): Promise<void> {
  return withMock(async () => {
    await api.delete(`/gestionnaire/remboursements/${id}`)
  }, undefined)
}
