...
export type LigneBudget = {
  ...
  compte_pcg: string
  libelle: string
  type: 'charge_courante' | 'charge_travaux' | 'produit_courant' | 'produit_travaux'
  realise_n1: number
  budget_n: number
  engagement: number
  realise: number
...
const MOCK_LIGNES_BUDGET: LigneBudget[] = [
  { id: 1, budget_id: 10, compte_pcg: '6111', libelle: 'Gardiennage/Surveillance', type: 'charge_courante', realise_n1: 38000, budget_n: 42000, engagement: 7000, realise: 14000, pct_consomme: 33, ordre: 1 },
  { id: 2, budget_id: 10, compte_pcg: '6131', libelle: 'Maintenance ascenseur', type: 'charge_courante', realise_n1: 28000, budget_n: 32000, engagement: 5600, realise: 11200, pct_consomme: 35, ordre: 2 },
  { id: 3, budget_id: 10, compte_pcg: '6140', libelle: 'Électricité parties communes', type: 'charge_courante', realise_n1: 12000, budget_n: 14000, engagement: 2400, realise: 4800, pct_consomme: 34, ordre: 3 },
  { id: 4, budget_id: 10, compte_pcg: '6161', libelle: 'Assurances', type: 'charge_courante', realise_n1: 20000, budget_n: 22000, engagement: 2400, realise: 2400, pct_consomme: 11, ordre: 4 },
  { id: 5, budget_id: 10, compte_pcg: '6171', libelle: 'Nettoyage parties communes', type: 'charge_courante', realise_n1: 16000, budget_n: 18000, engagement: 3000, realise: 6000, pct_consomme: 33, ordre: 5 },
  { id: 6, budget_id: 10, compte_pcg: '6138', libelle: 'Ravalement de façade', type: 'charge_travaux', realise_n1: 0, budget_n: 50000, engagement: 15000, realise: 0, pct_consomme: 0, ordre: 6 },
  { id: 7, budget_id: 10, compte_pcg: '6131', libelle: 'Réfection toiture', type: 'charge_travaux', realise_n1: 0, budget_n: 30000, engagement: 0, realise: 0, pct_consomme: 0, ordre: 7 },
  { id: 8, budget_id: 10, compte_pcg: '7111', libelle: 'Cotisations copropriétaires', type: 'produit_courant', realise_n1: 114000, budget_n: 128000, engagement: 0, realise: 38400, pct_consomme: 30, ordre: 8 },
  { id: 9, budget_id: 10, compte_pcg: '7113', libelle: 'Appels de fonds travaux', type: 'produit_travaux', realise_n1: 0, budget_n: 80000, engagement: 0, realise: 0, pct_consomme: 0, ordre: 9 },
]
...
const MOCK_SIMULATION: SimulationCotisation = {
  ...
  lots: [
    { lot_numero: 'A-01', coproprietaire_nom: 'Hassan Benali', tantieme: 45, pct: 4.5, cotisation_annuelle: 5760, cotisation_mensuelle: 480, variation_vs_n1: 8.5 },
    { lot_numero: 'A-02', coproprietaire_nom: 'Fatima Chraibi', tantieme: 42, pct: 4.2, cotisation_annuelle: 5376, cotisation_mensuelle: 448, variation_vs_n1: 8.5 },
    { lot_numero: 'A-03', coproprietaire_nom: 'Youssef Tazi', tantieme: 38, pct: 3.8, cotisation_annuelle: 4864, cotisation_mensuelle: 405, variation_vs_n1: 8.5 },
    { lot_numero: 'A-04', coproprietaire_nom: 'Nadia Berrada', tantieme: 38, pct: 3.8, cotisation_annuelle: 4864, cotisation_mensuelle: 405, variation_vs_n1: 8.5 },
    { lot_numero: 'A-05', coproprietaire_nom: 'Omar Fassi', tantieme: 35, pct: 3.5, cotisation_annuelle: 4480, cotisation_mensuelle: 373, variation_vs_n1: 8.5 },
  ],
}

const MOCK_SUGGESTIONS_IA: SuggestionIa[] = [
  { compte_pcg: '6111', libelle: 'Gardiennage/Surveillance', montant_suggere: 44000, montant_n1: 38000, variation_pct: 15.8, justification: 'Hausse du SMIG 2026 impactant le coût des agents de sécurité (+15%)' },
  { compte_pcg: '6131', libelle: 'Maintenance ascenseur', montant_suggere: 33500, montant_n1: 28000, variation_pct: 19.6, justification: 'Contrat Kone Maroc renouvelé avec révision annuelle de 5% + visite réglementaire supplémentaire' },
  { compte_pcg: '6140', libelle: 'Électricité', montant_suggere: 15000, montant_n1: 12000, variation_pct: 25.0, justification: "Révision des tarifs ONEE prévue au T2 2026 (+15%) + installation d'éclairage LED (ROI 18 mois)" },
  { compte_pcg: '6161', libelle: 'Assurances', montant_suggere: 23000, montant_n1: 20000, variation_pct: 15.0, justification: 'Renouvellement police AXA avec extension RCO et majoration indice BTP 2026' },
  { compte_pcg: '6171', libelle: 'Nettoyage', montant_suggere: 18500, montant_n1: 16000, variation_pct: 15.6, justification: 'Appel d\'offres 2026 — 3 offres reçues, recommandation prestataire actuel + fréquence espaces verts' },
]

// ─── New service functions for Annexe 5 ──────────────────────────────────────

export async function getBudgetAnnexe5(residenceId: number, exerciceId: number): Promise<BudgetAnnexe5 | null> {
  return withMock(async () => {
...
export async function updateLigneBudget(ligneId: number, budget_n: number): Promise<LigneBudget> {
  return withMock(async () => {
    ...
    ...(MOCK_LIGNES_BUDGET.find((l) => l.id === ligneId) ?? MOCK_LIGNES_BUDGET[0]),
    budget_n,
    pct_consomme: budget_n > 0
      ? Math.round(((MOCK_LIGNES_BUDGET.find((l) => l.id === ligneId)?.realise ?? 0) / budget_n) * 100)
      : 0,
  })
}
...
export async function bulkUpdateLignes(
  ...
  return withMock(async () => {
    ...
    ...(MOCK_LIGNES_BUDGET.find((ml) => ml.id === l.id) ?? MOCK_LIGNES_BUDGET[0]),
    budget_n: l.budget_n,
  })))
}

export async function ajouterLigne(budgetId: number, data: Partial<LigneBudget>): Promise<LigneBudget> {
  return withMock(async () => {
...
export async function soumettreBudgetAg(budgetId: number): Promise<BudgetAnnexe5> {
  return withMock(async () => {
...
export async function verrouillerBudget(budgetId: number): Promise<BudgetAnnexe5> {
  return withMock(async () => {
    const res = await api.post<ApiEnvelope<BudgetAnnexe5>>(
...
export async function getSimulationCotisation(budgetId: number): Promise<SimulationCotisation> {
  return withMock(async () => {
...
export async function getSuggestionsIa(budgetId: number): Promise<SuggestionIa[]> {
  return withMock(async () => {
