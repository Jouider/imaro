...
export async function runComplianceAudit(
  residenceId: number, exercice: number,
): Promise<ComplianceAudit> {
  return withMock(
    async () => {
      const res = await api.post<ApiEnvelope<ComplianceAudit>>(
        '/ia/compliance-audit', { residence_id: residenceId, exercice },
      )
...
export type InvoiceCategory =
  | 'eau' | 'electricite' | 'gaz' | 'fluides'
  | 'nettoyage' | 'gardiennage' | 'maintenance' | 'ascenseur'
  | 'syndic_honoraires' | 'assurance' | 'banque'
  | 'impots_taxes' | 'salaires_charges' | 'fournitures' | 'autre'
...
export async function extractInvoiceData(file: File): Promise<InvoiceExtraction> {
  return withMock(
    async () => {
      const formData = new FormData()
      formData.append('file', file)
      const res = await api.post<ApiEnvelope<InvoiceExtraction>>(
        '/ia/extract-invoice', formData,
...
const MOCK_BUDGET_SUGGESTION: BudgetSuggestion = {
  ...
  lignes: [
    {
      compte_numero: '6111', compte_libelle: 'Eau — compteur général',
      realise_n_minus_2: 3800, realise_n_minus_1: 4100, realise_n_partiel: 3650,
      suggestion_n_plus_1: 4300, variation_pct: 4.9,
      justification: 'Hausse tarif Lydec attendue + consommation stable',
      confidence: 'high',
    },
    {
      compte_numero: '6112', compte_libelle: 'Électricité',
      realise_n_minus_2: 5200, realise_n_minus_1: 5800, realise_n_partiel: 5100,
      suggestion_n_plus_1: 6100, variation_pct: 5.2,
      justification: "Hausse tarif Redal + LED non encore installées dans le sous-sol",
      confidence: 'high',
    },
    {
      compte_numero: '6131', compte_libelle: 'Nettoyage parties communes',
      realise_n_minus_2: 7200, realise_n_minus_1: 7800, realise_n_partiel: 6900,
      suggestion_n_plus_1: 7800, variation_pct: 0,
      justification: 'Contrat Clean Pro Maroc reconduit au même tarif',
      confidence: 'high',
    },
    {
      compte_numero: '6134', compte_libelle: 'Contrats de maintenance',
      realise_n_minus_2: 9600, realise_n_minus_1: 10200, realise_n_partiel: 9450,
      suggestion_n_plus_1: 10700, variation_pct: 4.9,
      justification: 'TechElev: +5% (annoncé). Plomberie: stable. Vidéo: stable.',
      confidence: 'medium',
    },
    {
      compte_numero: '6137', compte_libelle: 'Honoraires du syndic',
      realise_n_minus_2: 1800, realise_n_minus_1: 1800, realise_n_partiel: 1500,
      suggestion_n_plus_1: 1900, variation_pct: 5.6,
      justification: 'Indexation contractuelle annuelle',
      confidence: 'high',
    },
    {
      compte_numero: '6136', compte_libelle: "Primes d'assurance",
      realise_n_minus_2: 2900, realise_n_minus_1: 3050, realise_n_partiel: 2800,
      suggestion_n_plus_1: 3200, variation_pct: 4.9,
      justification: 'AXA: revalorisation moyenne du secteur',
      confidence: 'medium',
    },
    {
      compte_numero: '6143', compte_libelle: 'Autres charges',
      realise_n_minus_2: 1800, realise_n_minus_1: 1450, realise_n_partiel: 1100,
      suggestion_n_plus_1: 1500, variation_pct: 3.4,
      justification: "Inflation générale, retour à la moyenne historique",
      confidence: 'low',
    },
    {
      compte_numero: '6161', compte_libelle: 'Impôts et taxes',
      realise_n_minus_2: 750, realise_n_minus_1: 800, realise_n_partiel: 800,
      suggestion_n_plus_1: 830, variation_pct: 3.8,
      justification: 'Taxe sur les terrains urbains non bâtis : revalorisation cadastrale',
      confidence: 'medium',
    },
    {
      compte_numero: '6135', compte_libelle: 'Entretien courant et réparations',
      realise_n_minus_2: 3200, realise_n_minus_1: 3500, realise_n_partiel: 3100,
      suggestion_n_plus_1: 4100, variation_pct: 17.1,
      justification: "Lissage des réparations imprévues : provision conservatrice + état des équipements > 5 ans",
      confidence: 'low',
    },
  ],
  metadata: {
...
export async function suggestBudget(
  residenceId: number, exerciceCible: number,
): Promise<BudgetSuggestion> {
  return withMock(
    async () => {
      const res = await api.post<ApiEnvelope<BudgetSuggestion>>(
        '/ia/suggest-budget', { residence_id: residenceId, exercice: exerciceCible },
      )
      return res.data.data
    },
    { ...MOCK_BUDGET_SUGGESTION, residence_id: residenceId, exercice_cible: exerciceCible },
  )
}
