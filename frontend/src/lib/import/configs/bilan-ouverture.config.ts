...
export const bilanOuvertureConfig: ImportConfig<BilanOuverturePayload> = {
  ...
  validate(row: Record<string, unknown>): string[] {
    ...
    if (!numero) errors.push('Numéro de compte requis')
      ...
      errors.push(
        'Numéro invalide — doit commencer par 1-5 (Plan Comptable Marocain Syndic)',
      )
    }
    ...
    if (debitStr && !validateNumber(debitStr))
      errors.push('Solde débit invalide')
    if (creditStr && !validateNumber(creditStr))
      errors.push('Solde crédit invalide')

    if (debit === 0 && credit === 0) {
      errors.push(
        'Au moins un des soldes (débit ou crédit) doit être renseigné',
      )
    }
    if (debit > 0 && credit > 0) {
      errors.push(
        'Un compte ne peut avoir un solde à la fois en débit et en crédit',
      )
    }
    if (debit < 0 || credit < 0) {
  ...
  templateExampleRows: [
    ...
    {
      numero_compte: '3421',
      libelle: 'Copropriétaires — appels émis',
      solde_debit: 5300,
      solde_credit: 0,
    },
    {
