...
class AppelFondsResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            // relationLoaded() est vrai même si la relation chargée vaut null
            // (ex. appel sans exercice_id) → on garde-fou pour éviter un 500.
            'residence' => $this->when($this->relationLoaded('residence') && $this->residence, fn () => [
                'id' => $this->residence->id,
                'name' => $this->residence->name,
            ]),
            'exercice' => $this->when($this->relationLoaded('exercice') && $this->exercice, fn () => [
                'id' => $this->exercice->id,
