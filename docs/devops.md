...
# Restart Horizon prod
sudo supervisorctl restart horizon

# Restart Horizon staging
sudo supervisorctl restart horizon-staging

# Re-seed production (ATTENTION : efface toutes les donnees)
...
| Secret        | Valeur                        |
|---------------|-------------------------------|
| `VPS_HOST`    | 72.62.20.248                  |
| `VPS_USER`    | abdellah                      |
| `VPS_SSH_KEY` | Cle privee RSA CI/CD          |
| `VPS_PORT`    | 2222                          |
| `VITE_API_URL`| https://api.imaro.ma          |

Environments GitHub : `production` et `staging`.
